...
public class FirstPersonControllerEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        fpsController.walkSoundSpeed = EditorGUILayout.Slider(new GUIContent("Walk Playback", "Determines the speed at which footstep sounds will play while walking."), fpsController.walkSoundSpeed, 0.1f, 0.5f);
        if (fpsController.playerCanMove)
        {
            ...
            GUI.enabled = fpsController.playerCanSprint;
            ...
            fpsController.sprintSoundSpeed = EditorGUILayout.Slider(new GUIContent("Sprint Playback", "Determines the speed at which footstep sounds will play while sprinting."), fpsController.sprintSoundSpeed, 0.1f, 0.5f);
        ...
        GUI.enabled = fpsController.canJump;
        ...
        fpsController.groundCheckRadius = EditorGUILayout.Slider(new GUIContent("Radius Of Ground Check", "Sets the radius of the ground checking sphere."), fpsController.groundCheckRadius, 0.1f, 1f);
        fpsController.groundCheckTransform = (Transform)EditorGUILayout.ObjectField(new GUIContent("Ground Check Object's Transform", "Determines the position of ground checking sphere througha empty gameObject."), fpsController.groundCheckTransform, typeof(Transform), true);
        fpsController.gravitationalForce = EditorGUILayout.Slider(new GUIContent("Gravitation Force", "Sets the the gravitation force which will act on the player."), fpsController.gravitationalForce, 5f, 40f);
        EditorGUILayout.Space();
        ...
        GUI.enabled = fpsController.canCrouch;
        ...
        fpsController.crouchSoundPlayTime = EditorGUILayout.Slider(new GUIContent("Playback Speed", "Determines the speed at which footstep sounds will play while crouched."), fpsController.crouchSoundPlayTime, 0.1f, 0.5f);
        GUI.enabled = true;
        EditorGUILayout.Space();
        #endregion
        #region Camera Setup
        ...
        GUI.enabled = fpsController.canPush;
        ...
        GUI.enabled = fpsController.hasArmature;
